...
export class TradeItemSelectorSection extends TailwindElement {
    ...
    @property({ attribute: false })
    onItemsChange: ((items: Array<{ item: TradeItem; quantity: number }>) => void) | null = null;
                        ...
                        (place) => html`
                            <div class="p-4 border border-solid rounded-lg">
                                ...
                                ${place.items.map((item) => {
                                    const remainingMax = this.getRemainingItemMax(item);
                                        ...
                                        <div class="flex items-center gap-2 mb-2">
                                            <span class="flex-1">
                                                ...
                                                <span class="text-sm text-gray-500">
                                                    (남은 개수: ${remainingMax}개
                                                    ${usedQuantity > 0
                                                        ? html`/ 사용: ${usedQuantity}개`
                                                        : ""})
                                                </span>
                                            </span>
                                            <div class="flex items-center gap-1">
                                                <input
                                                    ...
                                                    value=${this.selectedItems.find(
                                                        (si) =>
                                                            si.item.name === item.name
                                                    )?.quantity || ""}
                                                    class="w-20 p-1 border border-solid rounded"
                                                    ?disabled=${remainingMax === 0}
                                                    @change=${(e: Event) => {
                                                        const input = e.target as HTMLInputElement;
                                                        const quantity = parseInt(input.value) || 0;
                                                        this.updateSelectedItems(item, quantity);
                                                    }}
                                                />
                                                <div class="flex gap-1">
                                                    <button
                                                        ...
                                                        ?disabled=${remainingMax === 0}
                                                        @click=${() => this.updateSelectedItems(item, remainingMax)}
                                                    >
                                                        최대
                                                    </button>
                                                    <button
                                                        ...
                                                        @click=${() => this.updateSelectedItems(item, 0)}
                                                    >
                                                        취소
                                                    </button>
                                                </div>
